...
**Map rendering** (`src/components/map/`): world node positions are fixed coordinates in `mapLayout.ts` (`MAP_LAYOUT`, a 900×320 viewBox) shared by `LevelPath` (the dashed SVG trail), `WorldNode` (per-world SVG icon + progress ring + lock state), and `Avatar` (tweens between world coordinates via Framer Motion). All map/world art is inline SVG or CSS — no image assets. Note: a `motion.g` that animates `y`/`rotate` via Framer Motion will silently override any static `transform` attribute on that same element — bake positional offsets into the animated values instead (see `Avatar.tsx`'s bob animation, which had this bug once).
